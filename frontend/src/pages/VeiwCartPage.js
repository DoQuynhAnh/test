...
const ViewCartPage = {
  async render() {
    ...
    <span class="count-san-pham">(${data ? data.length : 0} sản phẩm)</span>
    ...
    ${data
      ...
      <div class="item d-flex col-md-12"> 
        <div class="col-md-3"><a href="#"><img 
          src="http://localhost:4000/api/product/photo/${ele._id}"
        ></a></div>
          <div class="col-md-3">
        ...
        <div class="col-6">
          ...
          <label for=${ele._id} class="pr-3">Số Lượng :</label>
            <div class="d-flex">
              <input type="number" class="form-control-custom border-0" min="1" id=${`id${ele._id}`} value=${ele.count} >
              <button class="btn btn-primary uu" data-id=${ele._id}>
                Cập nhật
              </button>
          </div>
        </div>
  ...
  async afterRender() {
    ...
    removeBtn.forEach((item) => {
      item.addEventListener("click", async () => {
        ...
        if (item.classList.contains("delete-item")) {
          ...
          if (ask) {
            let data = JSON.parse(localStorage.getItem("cart"));
            let newData = data.filter((o) => o._id != targetBtn);
    ...
    btnUpdateCarts.forEach((a) => {
      ...
      a.addEventListener("click", async () => {
        let indexCart = await arr.map((ele) => ele._id).indexOf(id);
        const newCount = document.querySelector(`.form-group #id${id}`).value;
